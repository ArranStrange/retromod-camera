...
def draw_editor_panel(preview: np.ndarray, editor: ProfileEditor) -> np.ndarray:
    ...
    out = preview.copy()
    row_h, top, panel_w = 20, 46, 190
    panel_h = top + len(PARAMS) * row_h + 26
    ...
    hint_y = top + len(PARAMS) * row_h + 8
    cv2.putText(out, "[ ] param  - = adjust", (10, hint_y), _FONT, 0.36, _GRAY, 1, cv2.LINE_AA)
